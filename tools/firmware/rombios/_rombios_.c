...
print_bios_banner()
{
  bios_printf(2, "HVMAssist"" BIOS, %d cpu%s, ", 1, 1>1?"s":"");
  bios_printf(2, "%s %s\n", bios_cvs_version_string, bios_date_string);
  bios_printf(2, "TCG-enabled BIOS.\n");
  bios_printf(2, "\n");
}
struct ipl_entry {
  Bit16u type;
...
init_boot_vectors()
{
  ...
  Bit16u count = 0;
  Bit16u ss = get_SS();
  memsetb(0x9ff0, 0x0000, 0, 0xff);
  e.type = 1; e.flags = 0; e.vector = 0; e.description = 0; e.reserved = 0;
  memcpyb(0x9ff0, 0x0000 + count * sizeof (e), ss, &e, sizeof (e));
  count++;
  ...
  memcpyb(0x9ff0, 0x0000 + count * sizeof (e), ss, &e, sizeof (e));
  count++;
  write_word(0x9ff0, 0x0080, count);
  write_word(0x9ff0, 0x0082, 0xffff);
}
static Bit8u
get_boot_vector(i, e)
Bit16u i; struct ipl_entry *e;
{
  Bit16u count;
  Bit16u ss = get_SS();
  count = read_word(0x9ff0, 0x0080);
  if (i >= count) return 0;
  memcpyb(ss, e, 0x9ff0, 0x0000 + i * sizeof (*e), sizeof (*e));
  return 1;
}
  ...
  int selected;
{
    Bit8u scode;
    int max;
    max = read_word(0x9ff0, 0x0080);
    for(;;) {
        if (selected > max || selected < 1) selected = 1;
        clearscreen();
        bios_printf(2 | 4, "\n\n\n\n\n\n\n");
        bios_printf(2 | 4, "          Select boot device\n\n");
        bios_printf(2 | 4, "            1. Floppy\n");
        bios_printf(2 | 4, "            2. Hard drive\n");
        bios_printf(2 | 4, "            3. CD-ROM\n");
        if (max == 4)
            bios_printf(2 | 4, "            4. Network\n");
...
void ata_init( )
{
  ...
  for (device=0; device<(4*2); device++) {
    ...
    write_dword(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].sectors,0L);
    }
  for (device=0; device<(4*2); device++) {
...
void ata_detect( )
{
  ...
  for(device=0; device<(4*2); device++) {
    ...
    if(type == 0x02) {
      ...
      write_byte(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].mode, 0x00);
      if (ata_cmd_data_in(device,0xEC, 1, 0, 0, 0, 0L, get_SS(),buffer) !=0 )
      ...
      sectors = read_dword(get_SS(),buffer+(60*2));
      write_byte(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].device,0xFF);
      ...
      write_dword(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].sectors, sectors);
    ...
    if(type == 0x03) {
      ...
      write_byte(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].mode, 0x00);
      if (ata_cmd_data_in(device,0xA1, 1, 0, 0, 0, 0L, get_SS(),buffer) != 0)
      ...
      switch (type) {
        case 0x02:
          sizeinmb = read_dword(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].sectors);
          sizeinmb >>= 11;
        case 0x03:
      ...
      switch (type) {
        case 0x02:
          ...
          if (sizeinmb < 1UL<<16)
            bios_printf(2, " ATA-%d Hard-Disk (%04u MBytes)\n",version,(Bit16u)sizeinmb);
          else
            bios_printf(2, " ATA-%d Hard-Disk (%04u GBytes)\n",version,(Bit16u)(sizeinmb>>10));
          break;
        case 0x03:
          bios_printf(2, "ata%d %s: ",channel,slave?" slave":"master");
          i=0; while(c=read_byte(get_SS(),model+i++)) bios_printf(2, "%c",c);
...
Bit16u device;
{
  ...
  max=0xff;
  while(--max>0) {
    Bit8u status = inb(iobase1+7);
    if ((status & 0x80) != 0) break;
  }
  outb(iobase2+6, 0x08 | 0x02);
  if (read_byte(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].type) != 0x00) {
    outb(iobase1+6, slave?0xb0:0xa0);
    sc = inb(iobase1+2);
    sn = inb(iobase1+3);
    if ( (sc==0x01) && (sn==0x01) ) {
  ...
  Bit16u device;
{
  ...
  atacmd[0]=0x03;
  atacmd[4]=0x20;
  if (ata_cmd_packet(device, 12, get_SS(), atacmd, 0, 16L, 0x01, get_SS(), buffer) != 0)
    return 0x0002;
  ...
  Bit16u device;
{
  ...
  Bit8u buffer[];
  memsetb(get_SS(),atacmd,0,12);
  if (ata_cmd_packet(device, 12, get_SS(), atacmd, 0, 0L, 0x00, get_SS(), buffer) != 0)
    return 0x000f;
...
int1586_tick_end:
    ...
    case 0xe8:
        ...
        {
        case 0x20: {
            e820_table_size = read_word(0xe000, 0x8) * 0x14;
  ...
  Bit16u DI, SI, BP, SP, BX, DX, CX, AX;
{
  ...
  switch (scancode) {
    ...
    default:
      ...
      if (scancode > 0x58) {
        bios_printf(4, "KBD: int09h_handler(): unknown scancode (%x) read!\n", scancode);
        return;
        }
      if (shift_flags & 0x08) {
  ...
  switch (( AX >> 8 )) {
    ...
    case 0x04:
      ...
      if ( (count > 128) || (count == 0) ) {
        bios_printf(4, "int13_harddisk: function %02x, count out of range!\n",( AX >> 8 ));
        goto int13_fail;
        }
      ...
      if ( ( AX >> 8 ) == 0x02 )
        status=ata_cmd_data_in(device, 0x20, count, cylinder, head, sector, lba, segment, offset);
      else
        status=ata_cmd_data_out(device, 0x30, count, cylinder, head, sector, lba, segment, offset);
    ...
    case 0x47:
      ...
      if (lba != 0L) {
        bios_printf((2 | 4 | 1), "int13_harddisk: function %02x. Can't use 64bits lba\n",( AX >> 8 ));
        goto int13_fail;
        }
      lba=read_dword(DS, SI+(Bit16u)&((int13ext_t *) 0)->lba1);
      if (lba >= read_dword(ebda_seg, &((ebda_data_t *) 0)->ata.devices[device].sectors) ) {
      ...
      if (( ( AX >> 8 ) == 0x44 ) || ( ( AX >> 8 ) == 0x47 ))
        goto int13_success;
      if ( ( AX >> 8 ) == 0x42 )
        status=ata_cmd_data_in(device, 0x20, count, 0, 0, 0, lba, segment, offset);
      else
        status=ata_cmd_data_out(device, 0x30, count, 0, 0, 0, lba, segment, offset);
    ...
    case 0x48:
      ...
      if(size >= 0x1a) {
        ...
        nph = read_word(ebda_seg, &((ebda_data_t *) 0)->ata.devices[device].pchs.heads);
        npspt = read_word(ebda_seg, &((ebda_data_t *) 0)->ata.devices[device].pchs.spt);
        lba = read_dword(ebda_seg, &((ebda_data_t *) 0)->ata.devices[device].sectors);
        ...
        write_dword(DS, SI+(Bit16u)&((dpt_t *) 0)->sector_count1, lba);
        write_dword(DS, SI+(Bit16u)&((dpt_t *) 0)->sector_count2, 0L);
      ...
      if(size >= 0x1e) {
        ...
        write_word(ebda_seg, &((ebda_data_t *) 0)->ata.dpte.iobase2, iobase2);
        ...
        checksum=0;
        for (i=0; i<15; i++) checksum+=read_byte(ebda_seg, (&((ebda_data_t *) 0)->ata.dpte) + i);
        checksum = ~checksum;
    ...
    case 0x11:
    case 0x14:
      bios_printf(4, "int13h_harddisk function %02xh unimplemented, returns success\n", ( AX >> 8 ));
      goto int13_success;
      break;
    case 0x0a:
    case 0x0b:
  ...
  Bit16u drive;
{
  ...
  val8 = inb(0x3f4);
  if ( (val8 & 0xf0) != 0x80 )
    bios_printf((2 | 4 | 1), "floppy recal:f07: ctrl not ready\n");
  outb(0x03f5, 0x07);
  outb(0x03f5, drive);
 #asm
  sti
...
bios_printf(4, "floppy: drive>1 || head>1 ...\n");
        AX = ((AX & 0x00ff) | ((1) << 8));
        set_diskette_ret_status(1);
        AX = ((AX & 0xff00) | (0));
        FLAGS |= 0x0001;
  ...
  ;
        ...
        val8 = inb(0x3f4);
        if ( (val8 & 0xf0) != 0x80 )
          bios_printf((2 | 4 | 1), "int13_diskette:f02: ctrl not ready\n");
        outb(0x03f5, 0xe6);
  ...
  #asm
        ...
        val8 = inb(0x3f4);
        if ( (val8 & 0xf0) != 0x80 )
          bios_printf((2 | 4 | 1), "int13_diskette:f03: ctrl not ready\n");
        outb(0x03f5, 0xc5);
        outb(0x03f5, (head << 2) | drive);
        outb(0x03f5, track);
        outb(0x03f5, head);
...
;
      ...
      val8 = inb(0x3f4);
      if ( (val8 & 0xf0) != 0x80 )
        bios_printf((2 | 4 | 1), "int13_diskette:f05: ctrl not ready\n");
      outb(0x03f5, 0x4d);
      outb(0x03f5, (head << 2) | drive);
      outb(0x03f5, 2);
...
Bit16u seq_nr;
{
  ...
  bootdev >>= 4 * seq_nr;
  bootdev &= 0xf;
  if (bootdev == 0) bios_printf((2 | 4 | 1), "No bootable device.\n");
  bootdev -= 1;
...
int19_load_done:
    ...
    tcpa_ipl((Bit32u)0L,(Bit32u)bootseg,(Bit32u)0L,(Bit32u)512L);
  ...
  switch (regs.u.r8.ah) {
    ...
    case 0xb1:
      if (regs.u.r8.bl == 0xff) {
        ...
        bios_printf(4, "PCI device %04x:%04x not found\n", regs.u.r16.dx, regs.u.r16.cx);
      }
    ...
    if( ( registerC & 0x40 ) != 0 ) {
      if( read_byte( 0x40, 0xA0 ) != 0 ) {
        ...
        if( time < 0x3D1 ) {
          ...
          write_byte( segment, offset, 0x80 );
        } else {
          time -= 0x3D1;
...
ebda_ss_offset32()
{
    ...
    USE32
    #endasm
    return &((ebda_data_t *) 0)->upcall.reg_ss;
    #asm
    USE16
    #endasm
}
...
{
 Bit16u ebda_seg = read_word(0x0040, 0x000E);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.reg_ss, ss);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.reg_cs, cs);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.reg_ds, ds);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.reg_es, es);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.esp_hi, esp_hi);
}
  void
...
{
 Bit16u ebda_seg = read_word(0x0040, 0x000E);
 write_word(ebda_seg, &((ebda_data_t *) 0)->upcall.retaddr, retaddr);
}
Bit16u
get_returnaddress()
{
 return read_word_from_ebda(&((ebda_data_t *) 0)->upcall.retaddr);
}
Bit16u
get_register_cs()
{
 return read_word_from_ebda(&((ebda_data_t *) 0)->upcall.reg_cs);
}
Bit16u
get_register_ds()
{
 return read_word_from_ebda(&((ebda_data_t *) 0)->upcall.reg_ds);
}
Bit16u
get_register_es()
{
 return read_word_from_ebda(&((ebda_data_t *) 0)->upcall.reg_es);
}
Bit16u
get_register_esp_hi()
{
 return read_word_from_ebda(&((ebda_data_t *) 0)->upcall.esp_hi);
}
 #asm
Upcall:
